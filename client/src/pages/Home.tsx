...
const courseInfo: Record<string, { title: string; accent: string; description: string; tags: string[] }> = {
  root: {
    title: "The MIT CS constellation",
    accent: "pink",
    description: "A playful map of the computer science spine: start with foundations, branch into systems, then follow the signal toward intelligence.",
    tags: ["OCW INDEX", "MIT EECS", "SELF-PACED"],
  },
  foundations: {
    title: "Foundations",
    accent: "pink",
    description: "The core mental models: algorithms, linear algebra, probability, and the craft of making a proof executable.",
    tags: ["ALGORITHMS", "MATH", "LOGIC"],
  },
  "6.006": {
    title: "6.006 / Introduction to Algorithms",
    accent: "pink",
    description: "Design and analysis of efficient algorithms. A cozy little forest of graphs, heaps, hashing, and the occasional dynamic-programming boss battle.",
    tags: ["GRAPHS", "DP", "COMPLEXITY"],
  },
  "18.06": {
    title: "18.06 / Linear Algebra",
    accent: "pink",
    description: "Vectors, matrices, eigenvalues, and the geometry hiding under every good model.",
    tags: ["VECTORS", "EIGENVALUES", "MODELS"],
  },
  systems: {
  ...
  "6.034": {
    ...
    accent: "lime",
    description: "Symbolic reasoning, search, learning, and the lovely friction between a clean theory and a messy world.",
    tags: ["REASONING", "SEARCH", "LEARNING"],
  },
  "6.100": {
...
function Home() {
  ...
  const matchingApps = useMemo(() => {
    ...
    return apps.filter((app) => `${app.label} ${app.kicker}`.toLowerCase().includes(query));
  }, [commandQuery]);
  ...
  useEffect(() => {
    const onPointerMove = (event: PointerEvent) => {
      ...
      setPositions((current) => ({
        ...current,
        [drag.id]: { left: nextLeft, top: nextTop },
      }));
    };
  ...
  const bringToFront = (id: AppId) => {
    setActiveApp(id);
    setStackLevel((current) => current + 1);
  };
  ...
  const openApp = (id: AppId) => {
    ...
    setSelectedIcon(id);
    bringToFront(id);
    setSystemMessage(`${apps.find((app) => app.id === id)?.label} active`);
  };
  ...
  const runTerminalCommand = (event: FormEvent) => {
    ...
    if (command === "help") {
      output = ["available → whoami · contact · resume · open [app] · clear · status"];
    } else if (command === "whoami") {
      output = ["tejas / systems builder / MIT '26 / human-scale optimist"];
    } else if (command === "contact") {
      output = ["signal route → hello@tejas.dev  /  linkedin.com/in/tejas"];
    } else if (command === "resume") {
      output = ["resume.pdf is staged in the portfolio vault → opening Projects.exe"];
    } else if (command === "status") {
      output = ["cpu 18% · memory 42% · uptime 47d 03h · curiosity 100%"];
      ...
      const match = apps.find((app) => app.id === target || app.label.toLowerCase().startsWith(target));

      if (match) {
        openApp(match.id);
        output = [`launching ${match.label} …`];
      } else {
        output = [`app not found: ${target}`];
      }
    } else {
  ...
  const fetchApod = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(response.status === 429 ? "NASA rate limit reached. Add a personal key for more requests." : "NASA feed unavailable right now.");
      }
    ...
    return (
      <main className="welcome-screen">
        ...
        <div className="welcome-topline">
          <span>MIT POV OS / PERSONAL COMPUTING ENVIRONMENT</span>
          <span>BUILD 26.09.22</span>
        </div>
        ...
        <footer className="welcome-footer">
          ...
          <span>↓ the desktop is the portfolio</span>
    ...
    <main className="os-shell">
      <header className="topbar">
        <div className="topbar-brand">
          ...
          <span>MIT POV OS</span>
          <span className="brand-version">v2.6</span>
        </div>

        <div className="topbar-context">
          <span className="context-led" />
          {activeApp ? apps.find((app) => app.id === activeApp)?.label : "desktop"}
          <span className="slash">/</span>
          <span className="muted">focused workspace</span>
        </div>

        <div className="topbar-right">
          <span className="topbar-date">{formatDate(time)}</span>
          <span className="topbar-time">{formatTime(time)}</span>
          <button className="topbar-command" onClick={() => setCommandOpen(true)} aria-label="Open command palette">
            <Command size={14} />
            <kbd>⌘ K</kbd>
          </button>
        </div>
      ...
      <div className="desktop" ref={desktopRef} onPointerDown={() => setSelectedIcon(null)}>
        <div className="desktop-atmosphere atmosphere-one" />
        <div className="desktop-atmosphere atmosphere-two" />

        <div className="campus-silhouette" aria-hidden="true">
          <div className="dome" />
          <div className="dome-spire" />
          <div className="building building-a" />
          <div className="building building-b" />
          <div className="building building-c" />
        </div>

        <div className="desktop-header-note">
          <span className="eyebrow">MIT / CAMBRIDGE, MA</span>
          <h2>
            Make the
            <br />
        ...
        <div className="desktop-metrics">
          <div>
            ...
            <strong>{formatTime(time).slice(0, 5)}</strong>
          </div>
          <div>
        ...
        <aside className="desktop-sidebar" onPointerDown={(event) => event.stopPropagation()}>
          ...
          <div className="app-icon-grid">
            {apps.map(({ id, label, kicker, icon: Icon, tone }) => (
              <button
                key={id}
                className={`app-icon app-icon-${tone} ${selectedIcon === id ? "selected" : ""}`}
                onClick={(event) => {
              ...
              >
                ...
                <span className="app-icon-kicker">{kicker}</span>
              </button>
            ))}
          </div>
        ...
        <div className="window-layer" onPointerDown={(event) => event.stopPropagation()}>
          <WindowFrame
            id="transcript"
            title="Transcript.exe"
            eyebrow="academic record / encrypted"
            icon={BookOpen}
            isOpen={openWindows.transcript}
            isMaximized={maximized.transcript}
            position={positions.transcript}
            zIndex={maximized.transcript ? stackLevel + 2 : stackLevel}
            onClose={closeApp}
          ...
          >
            <div className="window-toolbar">
              <span className="toolbar-status">
                <span className="mini-led green" />
                VERIFIED ACADEMIC RECORD
              </span>
              <span className="toolbar-code">ID: AK-26-004</span>
            </div>

            <div className="transcript-hero">
              <div>
                <span className="micro-label">CUMULATIVE GPA</span>
                <strong>4.00</strong>
                <span className="transcript-rank">TOP 1% · CLASS OF 2026</span>
              </div>
            ...
            <div className="course-table">
              ...
              <div className="course-row">
                <span>SP '25</span>
                <span>
                  <b>6.033</b> Computer Systems Engineering
                </span>
                <strong>A</strong>
              </div>
              <div className="course-row">
                <span>FA '24</span>
                <span>
                  <b>6.006</b> Introduction to Algorithms
                </span>
                <strong>A</strong>
              </div>
              <div className="course-row">
              ...
              <div className="course-row">
                <span>FA '23</span>
                <span>
                  <b>6.100A</b> Intro to Programming
                </span>
                <strong>A</strong>
              </div>
            </div>
            ...
            <div className="transcript-footer">
              <span>
                <ShieldCheck size={13} />
                Registrar checksum verified
              </span>
              <span>04 / 04 terms shown</span>
            </div>
          </WindowFrame>

          <WindowFrame
            id="trophies"
            title="Trophy Case.exe"
            eyebrow="proof of work / 04 records"
            icon={Trophy}
            isOpen={openWindows.trophies}
            isMaximized={maximized.trophies}
            position={positions.trophies}
            zIndex={stackLevel}
            onClose={closeApp}
            onMaximize={toggleMaximize}
            onBringToFront={bringToFront}
          ...
          >
            <div className="award-intro">
              <div>
                <span className="micro-label">MOMENTUM INDEX</span>
                <strong>97.4</strong>
              </div>
              <p>
            ...
            <div className="award-list">
              {wins.map((award) => (
                <div className="award-row" key={award.badge}>
          ...
          <WindowFrame
            id="projects"
            title="Projects.exe"
            eyebrow="selected builds / open source"
            icon={FolderKanban}
            isOpen={openWindows.projects}
            isMaximized={maximized.projects}
          ...
          >
            <div className="projects-intro">
              <div>
                <span className="micro-label">BUILD QUEUE</span>
                <strong>03 SHIPPED</strong>
              </div>
          ...
          <WindowFrame
            id="ocw"
            title="OCW / CS Tree"
            eyebrow="MIT open courseware / constellation"
            icon={TreePine}
            isOpen={openWindows.ocw}
            isMaximized={maximized.ocw}
            position={positions.ocw}
            zIndex={stackLevel + 2}
            onClose={closeApp}
            onMaximize={toggleMaximize}
            onBringToFront={bringToFront}
            onPointerDown={startDrag}
          >
            <div className="ocw-intro">
              <div>
                <span className="micro-label">COURSE CONSTELLATION</span>
                <strong>THE CS TREE</strong>
              </div>
            ...
            <div className="ocw-layout">
              <div className="ocw-tree">
                {courseNodes.map((node) => (
                  <button
                    key={node.id}
                    className={`ocw-node ocw-${node.branch} ${ocwSelected === node.id ? "active" : ""}`}
                    style={{ "--depth": node.depth } as CSSProperties}
                    onClick={() => setOcwSelected(node.id)}
                  >
                    <span className="tree-branch">
                      {node.depth === 0 ? (
                        <TreePine size={14} />
                      ) : node.depth === 1 ? (
                        <span className="branch-joint">✦</span>
                      ) : (
                        <span className="leaf-dot" />
                      )}
                    </span>

                    <span className="ocw-node-copy">
                      <strong>{node.label}</strong>
                      <small>{node.meta}</small>
                    </span>

                    {node.depth === 2 && <span className="node-check">{node.meta.includes("completed") ? "✓" : "→"}</span>}
                  </button>
                ))}
              </div>

              <div className={`ocw-detail ocw-detail-${courseInfo[ocwSelected].accent}`}>
                <span className="micro-label">{courseInfo[ocwSelected].tags[0]}</span>
                <h3>{courseInfo[ocwSelected].title}</h3>
                <p>{courseInfo[ocwSelected].description}</p>

                <div className="ocw-tags">
                  {courseInfo[ocwSelected].tags.map((tag) => (
                    <span key={tag}>{tag}</span>
                  ))}
                </div>

                <a href="https://ocw.mit.edu/search/?q=computer+science" target="_blank" rel="noreferrer">
                  open MIT OCW <ExternalLink size={12} />
                </a>
              </div>
            </div>
          </WindowFrame>
          ...
          <WindowFrame
            id="terminal"
            title="Terminal.exe"
            eyebrow="command line / guest session"
            icon={TerminalIcon}
            isOpen={openWindows.terminal}
            isMaximized={maximized.terminal}
          ...
          >
            <div className="terminal-screen">
              {terminalLines.map((line, index) => (
                <div key={`${line}-${index}`} className={line.startsWith("guest") ? "terminal-command" : "terminal-output"}>
                  {line || "\u00a0"}
                </div>
              ))}

              <form className="terminal-form" onSubmit={runTerminalCommand}>
                <span>guest@mit-pov:~$</span>
                <input
                  autoFocus
                  value={terminalInput}
            ...
            <div className="terminal-status">
              ...
              <span>tab autocomplete · enter run</span>
            </div>
          </WindowFrame>
          ...
          <WindowFrame
            id="notes"
            title="Notes.exe"
            eyebrow="field notes / local vault"
            icon={FileText}
            isOpen={openWindows.notes}
            isMaximized={maximized.notes}
            position={positions.notes}
            zIndex={stackLevel}
            onClose={closeApp}
            onMaximize={toggleMaximize}
            onBringToFront={bringToFront}
          ...
          >
            <div className="notes-toolbar">
              ...
              <button onClick={saveNotes}>{notesSaved ? "saved" : "save changes"} <Download size={13} /></button>
            </div>
            ...
            <textarea
              ...
              aria-label="Editable personal notes"
            />

            <div className="notes-footer">
              <span>{notes.length} characters</span>
              <span>{notesSaved ? "local vault synced" : "unsaved changes"}</span>
            </div>
          </WindowFrame>
          ...
          <WindowFrame
            ...
            icon={Radar}
            isOpen={openWindows.orbital}
            isMaximized={maximized.orbital}
            position={positions.orbital}
            zIndex={stackLevel + 1}
            onClose={closeApp}
            onMaximize={toggleMaximize}
            onBringToFront={bringToFront}
          ...
          >
            <div className="orbital-intro">
              <div>
                <span className="micro-label">NASA / ASTRONOMY PICTURE OF THE DAY</span>
                <strong>LOOK UP.</strong>
              </div>
              <button className="refresh-button" onClick={fetchApod} disabled={apodLoading}>
                {apodLoading ? <Loader2 size={14} className="spin" /> : <RefreshCw size={14} />}
                {apodLoading ? "syncing" : "fetch latest"}
              </button>
            </div>

            <div className="nasa-key-row">
              <label htmlFor="nasa-key">API KEY</label>
              <input
                id="nasa-key"
                type="password"
                value={nasaKey}
                onChange={(event) => setNasaKey(event.target.value)}
                placeholder="optional · DEMO_KEY works"
              />
            ...
            {apod ? (
              <div className="apod-content">
                ...
                <div className="apod-copy">
                  ...
                  <a href={apod.hdurl || apod.url} target="_blank" rel="noreferrer">
                    open source image <ExternalLink size={13} />
                  </a>
                </div>
              </div>
            ) : (
              <div className="apod-empty">
                ...
                {apodError && <span className="error-message">{apodError}</span>}
              </div>
            )}
          </WindowFrame>
          ...
          <WindowFrame
            id="collab"
            title="Collab.exe"
            eyebrow="team room / live sync"
            icon={Network}
            isOpen={openWindows.collab}
            isMaximized={maximized.collab}
            position={positions.collab}
            zIndex={stackLevel + 2}
            onClose={closeApp}
            onMaximize={toggleMaximize}
            onBringToFront={bringToFront}
              ...
              <div className="collab-panel">
                ...
                <div className="member-list">
                  <div className="member-row">
                    <span className="member-avatar green">T</span>
                    <div>
                      <strong>Tejas</strong>
                      <small>editing system logic</small>
                    </div>
                    <span className="member-status online">online</span>
                  </div>

                  <div className="member-row">
                    <span className="member-avatar amber">LM</span>
                    <div>
                      <strong>Lena Moore</strong>
                      <small>reviewing notes</small>
                    </div>
                    <span className="member-status online">online</span>
                  </div>

                  <div className="member-row">
                    <span className="member-avatar blue">JS</span>
                    <div>
                      <strong>Jules Smith</strong>
                      <small>commenting on tasks</small>
                    </div>
                    <span className="member-status idle">idle</span>
                  </div>
                </div>
              </div>
              ...
              <div className="collab-panel">
                ...
                <div className="task-list">
                  <div className="task-item">
                    <div>
                      <strong>Finalize UI polish</strong>
                      <small>due in 45 mins</small>
                    </div>
                  ...
                  <div className="task-item">
                    <div>
                      <strong>Sync collaboration room</strong>
                      <small>in progress</small>
                    </div>
                    <span className="task-tag live">live</span>
                  </div>

                  <div className="task-item">
                    <div>
                      <strong>Review handoff notes</strong>
                      <small>queued</small>
                    </div>
                    <span className="task-tag queued">queued</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="collab-feed">
              ...
              <ul>
                <li>
                  ...
                  Tejas shared a new design pass in the room.
                </li>
                <li>
                  <span className="feed-dot" />
                  Lena left a review comment on the project brief.
                </li>
                <li>
                  <span className="feed-dot" />
                  Jules updated the shared checklist to “ready for QA”.
                </li>
              </ul>
            </div>
          </WindowFrame>
        ...
        <footer className="taskbar" onPointerDown={(event) => event.stopPropagation()}>
          <div className="taskbar-left">
            <button className="taskbar-launch" onClick={() => setCommandOpen(true)}>
              <Command size={15} />
              <span>LAUNCH</span>
            </button>
          ...
          <div className="taskbar-apps">
            {apps
              .filter((app) => openWindows[app.id])
              .map(({ id, label, icon: Icon }) => (
                <button
                  key={id}
                  className={`taskbar-app ${activeApp === id ? "active" : ""}`}
                  onClick={() => openApp(id)}
                >
                  <Icon size={14} />
                  {label.replace(".exe", "")}
        ...
        <div className="command-overlay" onMouseDown={() => setCommandOpen(false)}>
          <div className="command-palette" onMouseDown={(event) => event.stopPropagation()}>
            <div className="command-search">
              ...
              <input
                autoFocus
                placeholder="Jump to an app or command..."
                value={commandQuery}
            ...
            <div className="command-list">
              {matchingApps.map(({ id, label, kicker, icon: Icon, tone }) => (
                <button
                  key={id}
                ...
                >
                  <span className={`command-icon ${tone}`}>
                    <Icon size={16} />
                  </span>
                  <span>
                    <strong>{label}</strong>
              ...
              {matchingApps.length === 0 && (
                <div className="command-empty">No modules found. Try “terminal”, “notes”, or “NASA”.</div>
              )}
            </div>
...
function WindowFrame({
  ...
  return (
    <section
      id={`window-${id}`}
      className={`os-window ${variant} ${isMaximized ? "is-maximized" : ""}`}
      style={{ left: position.left, top: position.top, zIndex }}
      onPointerDown={() => onBringToFront(id)}
      aria-label={title}
    >
      <header className="window-header" onPointerDown={(event) => onPointerDown(event, id)}>
        ...
        <div className="window-controls">
          <button aria-label={`Minimize ${title}`} onPointerDown={(event) => event.stopPropagation()} onClick={() => onClose(id)}>
            <Minus size={13} />
          </button>
          <button aria-label={`Maximize ${title}`} onPointerDown={(event) => event.stopPropagation()} onClick={() => onMaximize(id)}>
            <Maximize2 size={12} />
          </button>
          <button
            className="close"
